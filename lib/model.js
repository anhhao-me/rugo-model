...
const Model = function(db, name, _schema){
  ...
  return {
    ...
    create: composeP(
      ...
      async (value) => {
        for (let entry in schema) {
          await validate(db(name), [entry, schema[entry]], value[entry]);
        }
    ...
    list: composeP(
      ...
      async (value) => {
        for (let entry in schema) {
          if (value[entry] !== undefined)
            await validate(db(name), [entry, schema[entry]], value[entry], { disableUniqueCheck: true });
        }
...
Model.use = (name, type) => {
  require('./transforms')[name.toLowerCase()] = type.transform;
  require('./validates')[name.toLowerCase()] = type.validate;
}
