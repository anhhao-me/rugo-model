  ...
  return name => {
    ...
    const handle = {
      ...
      async list(_query){
        ...
        query.dir = path.join(modelRoot, query.dir);

        const items = fs.existsSync(query.dir) ? fs.readdirSync(query.dir) : [];

        return {
          total: items.length,
          limit: -1,
          skip: 0,
          data: await Promise.all(items.map(name => getInfo(path.join(query.dir, name))))
        }
      },
      async get(_id){
